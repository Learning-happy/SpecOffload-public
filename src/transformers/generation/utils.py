...
import time
import threading
from .parallelism import MultiThreadGenerationInterface, AssistantProcessInterface
from .logger import LogConfig, print_tensor, print_tensor_match, print_log
...
class GenerationMixin:
    ...
    def generate(
        ...
        negative_prompt_ids: Optional[torch.Tensor] = None,
        negative_prompt_attention_mask: Optional[torch.Tensor] = None,
        multi_thread_interface: Optional[MultiThreadGenerationInterface] = None,
        assistant_process_interface: Optional[AssistantProcessInterface] = None,
        log_config: Optional[LogConfig] = None,
        num_of_new_tokens: Optional[List[int]] = None,
        **kwargs,
        ...
        if assistant_process_interface is not None:
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            if assistant_process_interface is None:
                candidate_model_kwargs = model_kwargs.copy()
                candidate_model_kwargs.pop("prefill_batch_size")
                candidate_generator = self._get_candidate_generator(
                    ...
                    assistant_tokenizer=assistant_tokenizer,
                    model_kwargs=candidate_model_kwargs,
                )
            else:
            ...
            result = self._assisted_decoding(
                ...
                streamer=streamer,
                multi_thread_interface=multi_thread_interface,
                assistant_process_interface=assistant_process_interface,
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        # self.model.bring_non_expert_to_gpu()
        # self.bring_lm_head_to_gpu()
        #! -----------------------------------------------------------------------------------
        # print(model_kwargs)
        pad_token_id = generation_config._pad_token_tensor
        output_attentions = generation_config.output_attentions
        ...
        while self._has_unfinished_sequences(
            ...
            if is_prefill:
                outputs = model_forward(**model_inputs, return_dict=True, is_decode=False)
                ...
                outputs = model_forward(**model_inputs, return_dict=True, is_decode=True)
                # outputs = model_forward(**model_inputs, return_dict=True)
        ...
        # print(f"prefill time: {prefill_end_time - start_time}")
        # print(f"decode time: {time.time() - prefill_end_time}")
    ...
    def _assisted_decoding(
        ...
        **sample** (depending on `do_sample`), assisted by candidate sequences. Assisted generation is an example of a
        ...
        # thread information
        thread_id = multi_thread_interface.thread_id if multi_thread_interface is not None else 0
        is_decode = False
        
        # count for generation
        n_generated = torch.zeros((batch_size), dtype=torch.long, device=input_ids.device)
        n_padding = torch.zeros((batch_size), dtype=torch.long, device=input_ids.device)

        # MaxLengthCriteria needs to be handled separately
        max_length_criteria = None
        for criteria in stopping_criteria:
            if isinstance(criteria, MaxLengthCriteria):
                max_length_criteria = criteria
                stopping_criteria.remove(criteria)
                break
        max_length = max_length_criteria.max_length if max_length_criteria is not None else None
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # Logging
            if log_config and (log_config.input_ids_shape or log_config.input_ids):
                print_log(f"input_ids: {input_ids.shape}", prefix="TOKEN", thread_id=thread_id, iteration=iter_count)
                if log_config.input_ids:
                    print_tensor(input_ids, n_padding=2, elem_format="5d")
            
            #  1. Fetch candidate sequences from a `CandidateGenerator`
            if not is_decode and assistant_process_interface is not None:
                # If in prefilling stage, don't generate candidates
                candidate_input_ids = input_ids
                candidate_logits = None
            else:
                if assistant_process_interface is not None:
                    # If assistant model runs in a separate process, generate candidates through the interface
                    # Prepare new attention mask
                    attention_mask = model_kwargs.get("attention_mask", torch.ones_like(input_ids))
                    new_length = input_ids.shape[1] - attention_mask.shape[1]
                    if new_length > 0:
                        attention_mask = torch.cat([attention_mask, attention_mask.new_ones((attention_mask.shape[0], new_length))], dim=-1)
                    
                    # Wait for assistant process to be ready
                    assistant_process_interface.assistant_ready_event.wait()
                    assistant_process_interface.assistant_ready_event.clear()
                    
                    # Logging
                    if log_config and log_config.timer:
                        candidate_generation_start = time.time()
                        print_log(f"Start candidate generation, at {log_config.get_time():.2f}s", prefix="TIMER", thread_id=thread_id, iteration=iter_count)
                    
                    # Transmit input
                    assistant_process_interface.set_input_ids(input_ids)
                    assistant_process_interface.set_attention_mask(attention_mask)
                    assistant_process_interface.input_ready_event.set()
                    
                    # Wait for assistant process to generate candidates
                    assistant_process_interface.output_ready_event.wait()
                    
                    # Receive output
                    candidate_input_ids = assistant_process_interface.get_input_ids()
                    candidate_logits = assistant_process_interface.get_candidate_logits()
                    
                    assistant_process_interface.output_ready_event.clear()
                    assistant_process_interface.assistant_ready_event.set()
                else:
                    # If assistant model runs in the same process, generate candidates directly
                    # Prepare new attention mask
                    attention_mask = model_kwargs.get("attention_mask", torch.ones_like(input_ids))
                    new_length = input_ids.shape[1] - attention_mask.shape[1]
                    if new_length > 0:
                    ...
                    if log_config and log_config.timer:
                        candidate_generation_start = time.time()
                        print_log(f"Start candidate generation, at {log_config.get_time():.4f}s", prefix="TIMER", thread_id=thread_id, iteration=iter_count)
                    
                    candidate_input_ids, candidate_logits = candidate_generator.get_candidates(input_ids, attention_mask)
            
                # Logging
                if log_config and (log_config.input_ids_shape or log_config.input_ids):
                    print_log(f"candidate_input_ids: {candidate_input_ids.shape}", prefix="TOKEN", thread_id=thread_id, iteration=iter_count)
                    print_log(f"candidate_length: {candidate_input_ids.shape[1] - input_ids.shape[1]}", prefix="TOKEN", thread_id=thread_id, iteration=iter_count)

            candidate_input_ids = candidate_input_ids.to(input_ids.device)
            if candidate_logits is not None:
                candidate_logits = candidate_logits.to(input_ids.device)
            
            # Logging and multi-threading
            if log_config and log_config.timer and is_decode:
                candidate_generation_end = time.time()
                print_log(f"Candidate generation finished, at {log_config.get_time():.4f}s", prefix="TIMER", thread_id=thread_id, iteration=iter_count)
                print_log(f"Candidate generation duration: {candidate_generation_end - candidate_generation_start:.2f}s", prefix="TIMER", thread_id=thread_id, iteration=iter_count, color="BLUE", background="WHITE")
            if multi_thread_interface is not None:
                multi_thread_interface.wait_and_clear()
                
            if log_config and log_config.timer:
                generation_start = time.time()
                print_log(f"Start generation, at {log_config.get_time():.4f}s", prefix="TIMER", thread_id=thread_id, iteration=iter_count)
            ...
            # Logging
            if log_config and log_config.model_inputs:
                print_log("model_inputs:", prefix="MODEL_INPUT", thread_id=thread_id, iteration=iter_count)
                for key, value in model_inputs.items():
                    print(f"    {key}:", end=" ")
                    if isinstance(value, torch.Tensor):
                        print(value.shape)
                        if key in ['input_ids', 'attention_mask', 'position_ids', 'cache_position']:
                            width = 5 if key == 'input_ids' else 3
                            print_tensor(value, elem_format=f"{width}d", n_padding=6)
                    elif isinstance(value, DynamicCache):
                        print("DynamicCache")
                        if len(value.key_cache) > 0:
                            print(f"        k: {len(value.key_cache)} * [ {value.key_cache[0].shape} ]")
                            if log_config.kv_cache:
                                for i in range(4):
                                    print(f"          batch {i}")
                                    print_tensor(value.key_cache[0][i, 0, :, :], elem_format=" .2e", n_padding=10, n_start_line=1, n_end_line=6, n_start_elem=4, n_end_elem=4)
                            print(f"        v: {len(value.value_cache)} * [ {value.value_cache[0].shape} ]")
                        else:
                            print("        k: []")
                            print("        v: []")
            ...
            outputs = self(**model_inputs, is_decode=is_decode)
            
            # Logging
            if log_config and log_config.kv_cache:
                print_log("(After forward pass) past_key_values:", prefix="KV_CACHE", thread_id=thread_id, iteration=iter_count)
                past_key_values = outputs.past_key_values
                print(f"  keys: {len(past_key_values.key_cache)} * [ {past_key_values.key_cache[0].shape} ]")
                for i in range(4):
                    print(f"  batch {i}")
                    print_tensor(past_key_values.key_cache[0][i, 0, :, :], elem_format=" .2e", n_padding=2, n_start_line=1, n_end_line=6, n_start_elem=4, n_end_elem=4)
            ...
            # mismatch, or until the max length is reached.
                ...
                # Only unfinished sequences generate new tokens
                n_matches = torch.where(unfinished_sequences.bool(), n_matches, torch.full_like(n_matches, -1))
                # For input sequences finishes in this iteration, determine the precise number of tokens to be generated
                for i in range(batch_size):
                    new_input_ids = torch.cat((input_ids[i:i+1], selected_tokens[i:i+1, :n_matches[i]+1]), dim=-1)
                    if unfinished_sequences[i] and stopping_criteria(new_input_ids, scores):
                        # Try to remove generated tokens from the end one by one, until the sequence is no longer finished
                        while stopping_criteria(new_input_ids[:, :-1], scores):
                            n_matches[i] -= 1
                            new_input_ids = new_input_ids[:, :-1]
                        unfinished_sequences[i] = 0
                
                valid_tokens = selected_tokens[:, : n_matches.max() + 1]
                n_generated += n_matches + 1
                
                # Logging
                if log_config and (log_config.input_ids_shape or log_config.generation_process):
                    if log_config.generation_statistics:
                        print_log(f"n_matches: {n_matches}", prefix="TOKEN", thread_id=thread_id, iteration=iter_count)
                        
                        counts = torch.masked_select(n_matches, n_matches >= 0).bincount()
                        print_log(f"n_matches counts: {counts}", prefix="STAT")
                        average = n_matches.float().mean()
                        print_log(f"n_matches average: {average:.1f}", prefix="STAT")
                        
                        print_log(f"n_generated: {n_generated}", prefix="TOKEN", thread_id=thread_id, iteration=iter_count)
                        
                        min_tokens = n_generated.min()
                        max_tokens = n_generated.max()
                        print_log(f"n_generated min: {min_tokens}, max: {max_tokens}", prefix="STAT")
                        average = n_generated.float().mean()
                        print_log(f"n_generated average: {average:.1f}", prefix="STAT")
                        average_last_iter = n_matches.float().mean() + 1
                        print_log(f"n_generated average last iteration: {average_last_iter:.1f}", prefix="STAT")
                        average_per_iter = average / (iter_count + 1)
                        print_log(f"n_generated average per iteration: {average_per_iter:.1f}", prefix="STAT")
                    else:
                        print_log(f"n_matches:", prefix="TOKEN", thread_id=thread_id, iteration=iter_count, end=" ")
                        print_tensor(n_matches, n_start_elem=4, n_end_elem=4)
                        
                        print_log(f"n_generated:", prefix="TOKEN", thread_id=thread_id, iteration=iter_count, end=" ")
                        print_tensor(n_generated, n_start_elem=4, n_end_elem=4)
            
            if log_config and log_config.input_ids:
                print_log(f"candidate_new_tokens: {candidate_new_tokens.shape}", prefix="TOKEN", thread_id=thread_id, iteration=iter_count)
                print_tensor_match(candidate_new_tokens, n_matches, n_padding=2)
                
                print_log(f"valid_tokens: {valid_tokens.shape}", prefix="TOKEN", thread_id=thread_id, iteration=iter_count)
                print_tensor_match(valid_tokens, n_matches + 1, n_padding=2)
            ...
            outputs.past_key_values = _crop_past_key_values(self, outputs.past_key_values, max_length=new_cache_size)
            
            # Logging
            if log_config and log_config.kv_cache:
                print_log("(After cropping with new_cache_size) past_key_values:", prefix="KV_CACHE", thread_id=thread_id, iteration=iter_count)
                past_key_values = outputs.past_key_values
                print(f"  keys: {len(past_key_values.key_cache)} * [ {past_key_values.key_cache[0].shape} ]")
                for i in range(4):
                    print(f"  batch {i}")
            ...
            # 4.3. Padding and trimming
            if n_matches is not None:
                # Count the number of padding tokens
                shift = n_matches.max() - n_matches
                if log_config and log_config.kv_cache:
                    print_log(f"shift: {shift}", prefix="KV_CACHE", thread_id=thread_id, iteration=iter_count)
                
                # Left padding for input_ids and attention_mask
                for i in range(batch_size):
                    if shift[i] > 0:
                        input_ids[i][shift[i]:] = input_ids[i][:-shift[i]].clone()
                        input_ids[i][:shift[i]] = self.generation_config.pad_token_id if self.generation_config.pad_token_id is not None else 0 
                        
                        model_kwargs["attention_mask"][i][shift[i]:] = model_kwargs["attention_mask"][i][:-shift[i]].clone()
                        model_kwargs["attention_mask"][i][:shift[i]] = 0
                        
                        n_padding[i] += shift[i]

                # Crop past_key_values
                model_kwargs["past_key_values"] = _crop_past_key_values(self, model_kwargs["past_key_values"], shift=shift)
                
                if log_config and log_config.kv_cache:
                    print_log("(After cropping with shift) past_key_values:", prefix="KV_CACHE", thread_id=thread_id, iteration=iter_count)
                ...
                # Crop shared left padding
                left_cut = min(n_padding)
                if log_config and log_config.kv_cache:
                    print_log(f"left_cut: {left_cut}", prefix="KV_CACHE", thread_id=thread_id, iteration=iter_count)
                
                if left_cut > 0:
                    input_ids = input_ids[:, left_cut:]
                    model_kwargs["attention_mask"] = model_kwargs["attention_mask"][:, left_cut:]
                    model_kwargs["cache_position"] = model_kwargs["cache_position"] - left_cut
                    model_kwargs["past_key_values"] = _crop_past_key_values(self, model_kwargs["past_key_values"], left_cut=left_cut)
                    n_padding = n_padding - left_cut
                    
                    if log_config and log_config.kv_cache:
                        print_log("(After cropping with left_cut) past_key_values:", prefix="KV_CACHE", thread_id=thread_id, iteration=iter_count)
                        past_key_values = model_kwargs["past_key_values"]
                        print(f"  keys: {len(past_key_values.key_cache)} * [ {past_key_values.key_cache[0].shape} ]")
                        for i in range(4):
                            print(f"  batch {i}")
            ...
            if return_dict_in_generate:
                newly_added_length = n_matches.max() + 1
                if output_scores:
            ...
            if max_length is None:
                ...
                # The batch is considered finished when:
                # - A sequence meets stopping_criteria by generating EOS token, and
                # - All the other unfinished sequences exceed the max_length
                this_peer_finished = torch.all(torch.where(unfinished_sequences.bool(), new_cur_len - n_padding >= max_length, torch.ones_like(unfinished_sequences))).item()
                
                # - Additional condition: if the average number of tokens generated is greater than the max_length, stop
                this_peer_finished = this_peer_finished or (n_generated.float().mean() >= max_length)
            
            is_first_iteration = False
            
            # Logging, multi-threading and multi-processing
            if log_config and log_config.timer:
                generation_end = time.time()
                print_log(f"Generation finished, at {log_config.get_time():.4f}s", prefix="TIMER", thread_id=thread_id, iteration=iter_count)
                print_log(f"Generation duration: {generation_end - generation_start:.2f}s", prefix="TIMER", thread_id=thread_id, iteration=iter_count, color="BLUE", background="WHITE")
            if multi_thread_interface is not None:
                multi_thread_interface.set()
            if assistant_process_interface is not None and not is_decode:
                torch.cuda.empty_cache()
                assistant_process_interface.batch_finish_prefilling_events[thread_id].set()
                is_decode = True
            
            iter_count += 1

        if multi_thread_interface is not None:
            multi_thread_interface.set_finished()

        # Count total number of tokens generated
        num_of_new_tokens[thread_id] = sum(n_generated)

        # Trim output: remove leading padding
        output_list = []
        for i in range(batch_size):
            output_list.append(input_ids[i][n_padding[i]:])
        ...
        if (
            ...
            and hasattr(candidate_generator, "assistant_model")
