...
class MixtralSdpaAttention(MixtralAttention):
    ...
    def forward(
        ...
        # print("MixtralSdpaAttention.forward")
        ...
        #     print(f"past_key_value.device: {past_key_value.key_cache[0].device}")
        ...
        if past_key_value is not None:
            ...
            # print(f"value_states.device: {value_states.device}")
            # if past_key_value._seen_tokens:
            #     print(f"past_key_value.device: {past_key_value.key_cache[0].device}")
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        # print("MixtralSparseMoeBlock.forward")
        """ """
...
class MixtralDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        placeholder: Optional[MixtralSparseMoeBlock] = None,
        is_decode: Optional[bool] = False,
        debug_msg_prefix: Optional[str] = "",
        ...
        **kwargs,
        ...
        # ! 根据之前的规划，将 decoding 阶段将attention放入 CPU 中
        if is_decode:
            hidden_states = hidden_states.to("cpu")
            #! batch的时候，attention_mask也需要放入CPU中
        ...
        SAMPLE_INTERVAL = 16
        layer_print_debug = (is_decode and layer_index % SAMPLE_INTERVAL == 0)
        layer_prefix = f"{debug_msg_prefix} Lay.{layer_index}" if debug_msg_prefix else ""
        
        #! 多线程并行，一个线程加载placeholder， 一个线程在CPU/GPU上执行attention
        #! --------------------------------------------------------------------------
        layer_start = time.time()
        # if layer_print_debug:
        print(f"{layer_prefix}: Start layer forward")
        
        placeholder_load_thread = Thread(target=self.placeholder_load, args=(placeholder, layer_print_debug, layer_prefix, layer_start, gpu_device))
        placeholder_load_thread.start()
        # placeholder_load_thread.join()
        ...
        print(f"{layer_prefix}: Attention finished")
        print(f"{YELLOW}{layer_prefix}: Attention duration: {attention_end - layer_start:.2f}s{RESET}")
        ...
        hidden_states, router_logits = placeholder(hidden_states)
        hidden_states = residual + hidden_states
    ...
    def placeholder_load(self, placeholder, layer_print_debug = False, layer_prefix = "", layer_start = None, gpu_device = "cuda:0"):
        placeholder = placeholder.to(gpu_device)
        placeholder.load_state_dict(
            self.block_sparse_moe.state_dict()
        )
        ...
        RESET = "\033[0m"
        load_end = time.time()
        print(f"{layer_prefix}: FFN parameters loaded")
        print(f"{YELLOW}{layer_prefix}: FFN parameters load duration: {load_end - layer_start:.2f}s{RESET}")
...
class MixtralModel(MixtralPreTrainedModel):
    ...
    def __init__(self, config: MixtralConfig):
        ...
        self.gpu_device = "cuda:0" # default gpu device
        self.moe_placeholder = None

        # Initialize weights and apply final processing
        self.post_init()
        
    def set_gpu_device(self, gpu_device):
        self.gpu_device = gpu_device
    
    def init_moe_placeholder(self):
        self.moe_placeholder = copy.deepcopy(self.layers[0].block_sparse_moe).to(self.gpu_device)

    def bring_non_expert_to_gpu(self):
        self.embed_tokens.to(self.gpu_device)
        self.norm.to(self.gpu_device)

        #! 目前仅保留 expert 的参数在 CPU内存 上
        for layer in self.layers:
            layer.self_attn.to(self.gpu_device)
            layer.input_layernorm.to(self.gpu_device)
            layer.block_sparse_moe.gate.to(self.gpu_device)
            layer.post_attention_layernorm.to(self.gpu_device)


    def bring_attn_to_cpu(self):
        for layer in self.layers:
            layer.self_attn.to('cpu')
        #! 必须要有这一行代码，否则显存不会释放
        #! 强制清理GPU显存
        ...
        for index, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    use_cache=use_cache,
                    cache_position=cache_position,
                    placeholder=self.moe_placeholder,
                    is_decode=is_decode,
                    layer_index=index,
...
class MixtralForCausalLM(MixtralPreTrainedModel, GenerationMixin):
    ...
    def init_gpu_device(self, gpu_device):
        self.model.set_gpu_device(gpu_device)
        self.model.init_moe_placeholder()

    #! 将lm_head参数加载到gpu
    def bring_lm_head_to_gpu(self, device="cuda:0"):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        if not is_decode:
            ...
            assert prefill_batch_size > 0
            
            for i in range(0, batch_size, prefill_batch_size):
                start = i
                end = min(i + prefill_batch_size, batch_size)

                past_key_values_batch = DynamicCache()
                input_ids_batch = input_ids[start:end]
                position_ids_batch = position_ids[start:end]
                ...
                #! print hidden_state is in cpu or gpu
                # print(hidden_states.device)
                ...
                outputs["past_key_values"].transforGPU2CPU()
                past_key_values_list.append(outputs["past_key_values"])
                logits_list.append(logits)
            
            #! 将Attention参数转移到CPU上
            self.model.bring_attn_to_cpu()

            outputs["past_key_values"] = DynamicCache.from_batch_splits(past_key_values_list)
