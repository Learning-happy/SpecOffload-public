...
echo "Running flexgen for OpenAI HumanEval benchmark..."
echo "NOTE: Make sure you have the required environment set up."
echo "    0. Please fetch the flegen code at https://github.com/FMInference/FlexLLMGen"
...
if [ "$dataset" == "openai_humaneval" ]; then
    if [ "$modelType" == "8x7B" ]; then
        python benchmarks/baseline/flexgen.py --model mistralai/Mixtral-8x7B-v0.1 \
            --batch-size 64 --n-token 32 --dataset openai_humaneval
    elif [ "$modelType" == "8x22B" ]; then
        python benchmarks/baseline/flexgen.py --model mistralai/Mixtral-8x22B-v0.1 \
            --batch-size 32 --n-token 32 --dataset openai_humaneval
    else
    ...
    if [ "$modelType" == "8x7B" ]; then
        python benchmarks/baseline/flexgen.py --model mistralai/Mixtral-8x7B-v0.1 \
            --batch-size 48 --n-token 32 --dataset samsum
    elif [ "$modelType" == "8x22B" ]; then
        python benchmarks/baseline/flexgen.py --model mistralai/Mixtral-8x22B-v0.1 \
            --batch-size 22 --n-token 32 --dataset samsum
    else
        echo "Unsupported model type. Use '8x7B' or '8x22B'."
        exit 1
    fi
    ...
    if [ "$modelType" == "8x7B" ]; then
        python benchmarks/baseline/flexgen.py --model mistralai/Mixtral-8x7B-v0.1 \
            --batch-size 38 --n-token 32 --dataset summeval
    elif [ "$modelType" == "8x22B" ]; then
        python benchmarks/baseline/flexgen.py --model mistralai/Mixtral-8x22B-v0.1 \
            --batch-size 20 --n-token 32 --dataset summeval
    else
        echo "Unsupported model type. Use '8x7B' or '8x22B'."
        exit 1
    fi
    ...
    if [ "$modelType" == "8x7B" ]; then
        python benchmarks/baseline/flexgen.py --model mistralai/Mixtral-8x7B-v0.1 \
            --batch-size 56 --n-token 32 --dataset ceval_exam
    elif [ "$modelType" == "8x22B" ]; then
        python benchmarks/baseline/flexgen.py --model mistralai/Mixtral-8x22B-v0.1 \
            --batch-size 36 --n-token 32 --dataset ceval_exam
    else
        echo "Unsupported model type. Use '8x7B' or '8x22B'."
        exit 1
    fi
