# ! This file is a modified version of the original file from Fiddler Labs
# ！实现了基础的Batch功能，修改tokenize函数为处理正常文本
...
from loadDataset import DATASET_DICT, tokenize_text
...
class FiddlerMixtral:
    def __init__(self, args):
        ...
        torch.cuda.set_per_process_memory_fraction(0.5, device=self.dev)

        # ! 这个文件中不使用Beam Search，所以不需要这个变量
    ...
    def generate(self, text=None, output_token=20, input_token=None, DATASET="openai_humaneval", GENERATION_BATCH_SIZE=1):
        ...
        input_ids, position_ids = self.tokenize_from_dataset(DATASET=DATASET, GENERATION_BATCH_SIZE=GENERATION_BATCH_SIZE)
        ...
        for i_token in range(output_token):
            print(f"Token {i_token+1}/{output_token}")
            if is_decode:
            ...
            # Greedy search: select token with highest probability
        ...
        throughput = input_ids.shape[0] * output_token / (prefill_time + decode_time)
        
        # print("--------------------")
        # print(f"Input: {text}")
        # print(f"Output: {decode_strings[0]}")
        # print(f"Throughput: {throughput} tokens/s")
        with open("log.txt", "a") as f:
            f.write(f"DATASET: {DATASET}\n")
            f.write(f"Batch Size: {GENERATION_BATCH_SIZE}\n")
            f.write(f"Output Tokens: {output_token}\n")
            f.write(f"Throughput: {throughput} tokens/s\n")
            f.write("\n")

        return prefill_time, decode_time, 0# self.cnt_expert_hit / self.cnt_expert_all
    

    # TODO: static batch now，之后可以改成动态batch
    def tokenize(self, text):
        input_ids = []
        with open ("romeo-and-juliet.txt", "r") as romeo_file:
                romeo_text = romeo_file.read()

        for index in range(self.batch_size):
            encodings = self.tokenizer(romeo_text, return_tensors="pt")
            input_id = encodings.input_ids[:,index*512:index*512+512].to(self.dev)
    ...
    def tokenize_from_dataset(self,
                              DATASET,
                              INPUT_TOKEN_LENGTH_PAD = None,
                              INPUT_TOKEN_LENGTH_TRUNCATE = None,
        ...
        input_tokens = tokenize_text(self.tokenizer, **tokenization_config)['input_ids']
        input_tokens = input_tokens.to(device=self.dev)
        
        position_ids = torch.arange(
            0, input_tokens.shape[-1], dtype=torch.long, device=self.dev
        )
        position_ids = position_ids.unsqueeze(0).view(-1, input_tokens.shape[-1])
        
        return input_tokens, position_ids
    ...
    def mixtral_forward(self, input_ids, position_ids, is_decode):
        ...
        for i_layer, layer in enumerate(self.model.layers):
            ...
            if self.cpu_offload == 0:
                ...
                # print(f"CPU Expert: {cpu_experts}")
                # print(f"GPU Expert: {gpu_experts}")
