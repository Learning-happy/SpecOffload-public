...
echo "Running Accelerate for OpenAI HumanEval benchmark..."
...
if [ "$dataset" == "openai_humaneval" ]; then
    if [ "$modelType" == "8x7B" ]; then
        python benchmarks/baseline/acc.py --model mistralai/Mixtral-8x7B-v0.1 \
            --batch-size 102 --n-token 32 --dataset openai_humaneval
    elif [ "$modelType" == "8x22B" ]; then
        python benchmarks/baseline/acc.py --model mistralai/Mixtral-8x22B-v0.1 \
            --batch-size 55 --n-token 32 --dataset openai_humaneval
    else
    ...
    if [ "$modelType" == "8x7B" ]; then
        python benchmarks/baseline/acc.py --model mistralai/Mixtral-8x7B-v0.1 \
            --batch-size 68 --n-token 32 --dataset samsum
    elif [ "$modelType" == "8x22B" ]; then
        python benchmarks/baseline/acc.py --model mistralai/Mixtral-8x22B-v0.1 \
            --batch-size 38 --n-token 32 --dataset samsum
    else
        echo "Unsupported model type. Use '8x7B' or '8x22B'."
        exit 1
    fi
    ...
    if [ "$modelType" == "8x7B" ]; then
        python benchmarks/baseline/acc.py --model mistralai/Mixtral-8x7B-v0.1 \
            --batch-size 60 --n-token 32 --dataset summeval
    elif [ "$modelType" == "8x22B" ]; then
        python benchmarks/baseline/acc.py --model mistralai/Mixtral-8x22B-v0.1 \
    ...
    if [ "$modelType" == "8x7B" ]; then
        python benchmarks/baseline/acc.py --model mistralai/Mixtral-8x7B-v0.1 \
            --batch-size 88 --n-token 32 --dataset ceval_exam
    elif [ "$modelType" == "8x22B" ]; then
        python benchmarks/baseline/acc.py --model mistralai/Mixtral-8x22B-v0.1 \
            --batch-size 55 --n-token 32 --dataset ceval_exam
    else
        echo "Unsupported model type. Use '8x7B' or '8x22B'."
        exit 1
    fi
